...
export class WordService {
  ...
  constructor(@InjectRepository(Word) private readonly wordReposition:Repository<Word>,
              @InjectRepository(Hitoapi) private readonly HitoApiReposition:Repository<Hitoapi>,
  ...
  async needAudit(pageSize:number,pageNumber:number){
    const [data,count]= await this.wordReposition.createQueryBuilder()
  ...
  async getData(){
    debugger
   const {data}=await this.httpService.get('https://hitoapi.cc/sp').toPromise()
    if(data){
      const HitoapiData={
        origin_id:data.id,
      ...
      // 查找是否存在
      const result = await this.HitoApiReposition.find({ text:HitoapiData.text })
      if (result.length > 0) {
